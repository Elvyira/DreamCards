...
namespace BarcodeScanner.Webcam
{
    public class UnityWebcam : IWebcam
    {
        ...
        public Vector2 Size
        {
            get { return new Vector2(Webcam.width, Webcam.height); }
        }
        ...
        public UnityWebcam(ScannerSettings settings)
        {
            ...
            Webcam = new WebCamTexture(settings.WebcamDefaultDeviceName, settings.WebcamRequestedWidth, settings.WebcamRequestedHeight)
            {
                filterMode = settings.WebcamFilterMode
            };
        ...
        {
            return string.Format("[UnityWebcam] Camera: {2} | Resolution: {0}x{1} | Orientation: {3}", Width, Height, Webcam.deviceName,
                Webcam.videoRotationAngle + ":" + Webcam.videoVerticallyMirrored);
        }
    }
}